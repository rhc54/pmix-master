...
static bool check_node(pmix_nodeinfo_t *n1, pmix_nodeinfo_t *n2)
{
    if (NULL != n1->hostname && NULL != n2->hostname &&
        0 == strcmp(n1->hostname, n2->hostname)) {
        return true;
    }
                                           ...
                                           pmix_tma_t *tma)
{
    ...
    for (j=0; j < size; j++) {
        if (PMIX_CHECK_KEY(&iptr[j], PMIX_SESSION_ID)) {
            ...
            if (PMIX_SUCCESS != (rc = process_node_array(&iptr[j].value, &ncache, tma))) {
                PMIX_ERROR_LOG(rc);
                easy_destruct(&cache);
                easy_destruct(&ncache);
                return rc;
    ...
    kp2 = (pmix_kval_t*)pmix_list_remove_first(&cache);
    while (NULL != kp2) {
        kv = PMIX_NEW_TMA(pmix_kval_t, tma);
        kv->value = (pmix_value_t*)pmix_tma_malloc(tma, sizeof(pmix_value_t));
        PMIX_VALUE_XFER(rc, kv->value, kp2->value);
