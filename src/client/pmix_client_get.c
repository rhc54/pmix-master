                                   ...
                                   pmix_value_t **val)
{
    ...
    /* check the request and handle trivial cases */
    ...
    if (!PMIX_PEER_IS_EARLIER(pmix_client_globals.myserver, 3, 1, 100)) {
        ...
        if (pmix_check_node_info(key)) {
           ...
           /* see if they told us to get node info */
            ...
            /* if they didn't specify a host and the rank is valid, then
             * they are asking for info about the node upon which that
             * rank is located */
            if (NULL == hostname && UINT32_MAX == nodeid && PMIX_RANK_IS_VALID(p.rank)) {
                /* lookup the node where this rank is located */
            ...
            if (NULL != hostname || UINT32_MAX != nodeid) {
                 ...
                 * specified the target node and it is NOT us, then we
                 * have to get it from the hash component */
            ...
            if (NULL != hostname) {
                 ...
                 * a hostname - if the host is other than us, then it can only
                 * be in the hash component */
            ...
            if (UINT32_MAX != nodeid) {
                /* they did not provide the "node-info" attribute but did specify
                 * the nodeid - if the ID is other than us, then it can only
                 * be in the hash component */
        ...
        if (pmix_check_app_info(key)) {
            ...
            if (wantinfo) {
                /* missing the appnum - if the rank is valid, then they are
                 * looking for on the app to which that rank belongs */
                if (PMIX_RANK_IS_VALID(p.rank)) {
                    /* lookup the appnum for this rank */
                    ...
                    PMIX_INFO_LOAD(&iptr[ninfo], PMIX_APPNUM, &app, PMIX_UINT32);
                    copy = true;
                    goto doget;
                }
                /* if the rank isn't valid, then assume they are asking
                 * about our app */
                goto fastpath;
            } else if (UINT32_MAX != appnum) {  // wantinfo == false
                /* they did not provide the "app-info" attribute but did specify
                 * the appnum - if the ID is other than us, then we must let the
                 * hash component provide it */
                rc = _getfn_fastpath(&pmix_globals.myid, PMIX_APPNUM, NULL, 0, &ival);
                if (PMIX_SUCCESS == rc) {
                    PMIX_VALUE_GET_NUMBER(rc, ival, app, uint32_t);
                    if (PMIX_SUCCESS != rc) {
                ...
                // mark that they asked for app info
                nfo = ninfo + 1;
                PMIX_INFO_CREATE(iptr, nfo);
                for (n=0; n < ninfo; n++) {
                    PMIX_INFO_XFER(&iptr[n], &info[n]);
